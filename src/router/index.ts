        ...
        children: [
          ...
          },
          {
            ...
            children: [
              ...
              { path: 'download-speed-calculator', name: 'DownloadSpeedCalculator', component: () => import('@/views/Tools/NumberTool/DownloadSpeedCalculator.vue'), meta: { title: '下载速度计算器', icon: 'DownloadOutlined', description: '选择计算目标（下载时间 / 文件容量 / 下载速度），填写另外两个参数即可自动算出结果。下载时间支持时、分、秒分别输入，文件容量支持 B 到 TB，下载速度支持 Mbps、MB/s 等带宽与下载工具常用单位。', seoDescription: '在线下载速度计算器，根据文件容量、下载速度和下载时间三个参数自动换算，支持Mbps与MB/s单位互转，时间可按时分秒输入，快速估算文件下载耗时和所需带宽' } },
            ]
          },
          {
